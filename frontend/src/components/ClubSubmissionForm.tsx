...
export default function ClubSubmissionForm({ onSuccess }: ClubSubmissionFormProps) { // Changed to default export
  const navigate = useNavigate();
  const [formData, setFormData] = useState({
    name: '',
    location: '',
    ...
    group_size: '',
    logo_url: '',
    photos: [''], // Ensure at least one empty field for initial display
    amenities: [''], // Ensure at least one empty field for initial display
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // CityDropdown specific validation
  ...
  // Remove field for any array type (meeting_times, photos, amenities)
  const removeArrayField = (field: 'meeting_times' | 'photos' | 'amenities', index: number) => {
    const newArray = formData[field].filter((_, i) => i !== index);
    // Ensure there's always at least one empty field if array cannot be truly empty
    // For photos/amenities, allow empty if desired, but for input consistency, we might keep one.
    // For meeting_times, allow it to become empty as it's a generated list.
    if ((field === 'photos' || field === 'amenities') && newArray.length === 0) {
  ...
  return (
    <PageContainer>
      {/* Changed py-16 to pt-20 pb-12 for consistent top spacing and overall page padding */}
      <div className="min-h-screen w-full px-4 pt-20 pb-12 flex flex-col">
        ...
        <div className="relative z-10 max-w-2xl mx-auto w-full">
          <div className="bg-white/70 dark:bg-gray-800/70 backdrop-blur-xl rounded-2xl p-6 md:p-8 shadow-xl border border-white/20 dark:border-gray-700/20">
            {createClubMutation.isPending && (
              <div className="absolute inset-0 bg-white/50 dark:bg-gray-800/50 flex items-center justify-center z-50 rounded-2xl"> {/* Added rounded-2xl */}
                <div className="flex items-center space-x-2">
                  <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-primary-500"></div> {/* Blue to primary */}
                  <span className="text-gray-600 dark:text-gray-300">Submitting club...</span>
                </div>
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-6" noValidate>
              {/* Success Message */}
              {success && (
                <div className="mb-6 p-4 bg-green-500/10 border border-green-500/30 text-green-700 dark:text-green-300 rounded-lg flex items-center">
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                  </svg>
                  Running club submitted successfully! Redirecting to clubs page...
              ...
              {/* Error Message */}
              {error && (
                <div className="mb-6 p-4 bg-red-500/10 border border-red-500/30 text-red-700 dark:text-red-300 rounded-lg flex items-center whitespace-pre-wrap">
                  <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                  </svg>
                  {error}
              ...
              {/* Input Fields - Styles updated to match EventsPage/EventSubmissionForm */}
              <div>
              ...
              {/* --- NEW MEETING TIMES UI (Styling adjusted) --- */}
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                  Meeting Times (Select days/time and click '+ Add Meeting Time')
                </label>
                    ...
                    {daysOfWeek.map(day => (
                      <button
                        ...
                        className={`
                          py-2 px-3 rounded-lg text-sm font-medium transition-colors duration-200
                          ${currentSelectedDays.includes(day)
                            ? 'bg-primary-600 text-white hover:bg-primary-700' // Changed to primary
                            : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-600' // Softer gray
                          }
                        `}
                      >
                        {day}
                ...
                <button
                  type="button"
                  onClick={handleAddFormattedMeetingTime}
                  className="mt-2 text-sm text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300 flex items-center" // Changed to primary
                >
                  <svg className="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                  Add Meeting Time
                </button>
                      ...
                      {formData.meeting_times.map((time, index) => (
                        <li key={index} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg shadow-sm">
                          ...
                          <button
                            type="button"
                            onClick={() => removeArrayField('meeting_times', index)}
                            className="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 text-sm" // Changed to red palette
                          >
                            Remove
                          </button>
                        </li>
              ...
              <div>
                ...
                {formData.photos.map((photo, index) => (
                  <div key={index} className="flex gap-2 mt-2">
                    <input
                      ...
                      placeholder="Photo URL"
                    />
                    {formData.photos.length > 1 || photo.trim() !== '' ? ( // Show remove if >1 or if it's the last but not empty
                        <button
                            type="button"
                            onClick={() => removeArrayField('photos', index)}
                            className="px-3 py-2 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" // Changed to red palette
                        >
                            Remove
                        </button>
                    ) : null}
                ...
                <button
                  type="button"
                  onClick={() => addArrayField('photos')}
                  className="mt-2 text-sm text-primary-600 hover:text-primary-800 dark:text-primary-400 dark:hover:text-primary-300" // Changed to primary
                >
                  + Add Photo
                </button>
              </div>
                ...
                {formData.amenities.map((amenity, index) => (
                  <div key={index} className="flex gap-2 mt-2">
                    <input
                      ...
                      placeholder="e.g., Water stations, Parking, Changing rooms"
                    />
                    {formData.amenities.length > 1 || amenity.trim() !== '' ? ( // Show remove if >1 or if it's the last but not empty
                        <button
                            type="button"
                            onClick={() => removeArrayField('amenities', index)}
                            className="px-3 py-2 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300" // Changed to red palette
              ...
              {/* Submit Button - Styles updated to match EventsPage/HomePage */}
              <div>
                <button
                  type="submit"
                  disabled={createClubMutation.isPending}
