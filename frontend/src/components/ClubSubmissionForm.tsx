...
//     formDataToSend.append('event_name', formData.name);  // Make sure this is non-empty
//     formDataToSend.append('file_ext', fileExt);
//     formDataToSend.append('index', (i + 1).toString());
//     formDataToSend.append('file', file);  // Required UploadFile field
...
//           {/* Add Meeting Time Button - Standardized to match EventSubmissionForm's add button */}
//           <button
//             type="button"
...
//                   {file.name} {imageUploadProgress[idx] === 100 && '✓'}
//                 </span>
//               ))}
//             </div>
//           )}
...
//                       className="px-3 py-2 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 text-sm ml-2"
//                   >
//                       ×
//                   </button>
//               ) : null}
//             </div>
//           ))}
//           {/* Add Another Amenity button - Standardized to match EventSubmissionForm's add button */}
//           <button
//             type="button"
...
//             <svg className="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"> {/* Changed mr-2 */}
...
//         >
//           {createClubMutation.isPending ? 'Submitting...' : 'Submit Club'}
//         </button>
//       </form>
//     </div>
//   );
...
export default function ClubSubmissionForm({ onSuccess }: ClubSubmissionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // No longer validating formData.photos as URLs here, they come from upload
        ...
        <button
          ...
          className={`w-full py-3 px-6 text-white font-semibold rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-800 ${
            uploadingImages || createClubMutation.isPending
              ? 'bg-gray-400 cursor-not-allowed' // Neutral disabled state
